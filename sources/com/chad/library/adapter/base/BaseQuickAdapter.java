...
public abstract class BaseQuickAdapter<T, K extends BaseViewHolder> extends RecyclerView.Adapter<K> {
    ...
    public void setNewDiffData(BaseQuickDiffCallback<T> baseQuickDiffCallback, boolean z) {
        ...
        DiffUtil.calculateDiff(baseQuickDiffCallback, z).dispatchUpdatesTo((ListUpdateCallback) new BaseQuickAdapterListUpdateCallback(this));
        this.mData = baseQuickDiffCallback.getNewList();
    }

    public void setNewDiffData(DiffUtil.DiffResult diffResult, List<T> list) {
        if (getEmptyViewCount() == 1) {
            setNewData(list);
            return;
        }
    ...
    public int expandAll(int i, boolean z, boolean z2) {
        ...
        Object item2 = i2 < this.mData.size() ? getItem(i2) : null;
        IExpandable expandableItem = getExpandableItem(headerLayoutCount);
        if (expandableItem == null) {
            return 0;
        }
