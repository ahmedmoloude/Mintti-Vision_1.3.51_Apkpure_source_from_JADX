...
public class MonotonicCurveFit extends CurveFit {
    ...
    public void getPos(double d, double[] dArr) {
        ...
        if (this.mExtrapolate) {
            if (d <= dArr2[0]) {
                ...
                for (int i2 = 0; i2 < length2; i2++) {
                    dArr[i2] = this.f127mY[0][i2] + ((d - this.f126mT[0]) * this.mSlopeTemp[i2]);
                }
                return;
            }
            int i3 = length - 1;
            if (d >= dArr2[i3]) {
                ...
                while (i < length2) {
                    dArr[i] = this.f127mY[i3][i] + ((d - this.f126mT[i3]) * this.mSlopeTemp[i]);
                    i++;
                }
                return;
            }
    ...
    public void getPos(double d, float[] fArr) {
        ...
        if (this.mExtrapolate) {
            if (d <= dArr[0]) {
                ...
                for (int i2 = 0; i2 < length2; i2++) {
                    fArr[i2] = (float) (this.f127mY[0][i2] + ((d - this.f126mT[0]) * this.mSlopeTemp[i2]));
                }
                return;
            }
            int i3 = length - 1;
            if (d >= dArr[i3]) {
                ...
                while (i < length2) {
                    fArr[i] = (float) (this.f127mY[i3][i] + ((d - this.f126mT[i3]) * this.mSlopeTemp[i]));
                    i++;
                }
                return;
            }
            ...
            for (int i4 = 0; i4 < length2; i4++) {
                fArr[i4] = (float) this.f127mY[0][i4];
            }
            return;
        } else {
            int i5 = length - 1;
        ...
        while (i6 < length - 1) {
            if (d == this.f126mT[i6]) {
                for (int i7 = 0; i7 < length2; i7++) {
                    fArr[i7] = (float) this.f127mY[i6][i7];
                }
            }
            double[] dArr2 = this.f126mT;
            int i8 = i6 + 1;
    ...
    public double getPos(double d, int i) {
        ...
        if (this.mExtrapolate) {
            if (d <= dArr[0]) {
                return this.f127mY[0][i2] + ((d - dArr[0]) * getSlope(dArr[0], i2));
            }
            int i4 = length - 1;
            if (d >= dArr[i4]) {
                return this.f127mY[i4][i2] + ((d - dArr[i4]) * getSlope(dArr[i4], i2));
            }
        } else if (d <= dArr[0]) {
