...
import p028io.reactivex.observers.DisposableObserver;
import p028io.reactivex.observers.SerializedObserver;
import p028io.reactivex.plugins.RxJavaPlugins;
import p028io.reactivex.subjects.UnicastSubject;
...
public final class ObservableWindowBoundarySelector<T, B, V> extends AbstractObservableWithUpstream<T, Observable<T>> {
    ...
    static final class WindowBoundaryMainObserver<T, B, V> extends QueueDrainObserver<T, Object, Observable<T>> implements Disposable {
        ...
        /* renamed from: ws */
        final List<UnicastSubject<T>> f2087ws;
        ...
        public void onNext(T t) {
            if (fastEnter()) {
                for (UnicastSubject<T> onNext : this.f2087ws) {
                    onNext.onNext(t);
                }
                if (leave(-1) == 0) {
                    return;
        ...
        public void drainLoop() {
            ...
            List<UnicastSubject<T>> list = this.f2087ws;
            int i = 1;
            while (true) {
                boolean z = this.done;
                ...
                if (z && z2) {
                    ...
                    if (windowOperation.f2088w != null) {
                        if (list.remove(windowOperation.f2088w)) {
                            windowOperation.f2088w.onComplete();
                            if (this.windows.decrementAndGet() == 0) {
                                disposeBoundary();
                                return;
                            }
        ...
        public void close(OperatorWindowBoundaryCloseObserver<T, V> operatorWindowBoundaryCloseObserver) {
            ...
            this.queue.offer(new WindowOperation(operatorWindowBoundaryCloseObserver.f2086w, null));
            if (enter()) {
                drainLoop();
            }
        }
    ...
    static final class WindowOperation<T, B> {
        ...
        /* renamed from: w */
        final UnicastSubject<T> f2088w;

        WindowOperation(UnicastSubject<T> unicastSubject, B b) {
            this.f2088w = unicastSubject;
            this.open = b;
        }
    }
    ...
    static final class OperatorWindowBoundaryCloseObserver<T, V> extends DisposableObserver<V> {
        ...
        /* renamed from: w */
        final UnicastSubject<T> f2086w;

        OperatorWindowBoundaryCloseObserver(WindowBoundaryMainObserver<T, ?, V> windowBoundaryMainObserver, UnicastSubject<T> unicastSubject) {
            this.parent = windowBoundaryMainObserver;
            this.f2086w = unicastSubject;
        }
